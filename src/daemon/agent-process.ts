...
export class AgentProcess {
  ...
  private buildStartupPrompt(): string {
    ...
    const handoffUxOverride = isHandoffRestart
      ? ' HANDOFF UX: This is a context handoff restart — your memory is intact via the handoff doc. CRITICAL: After reading the handoff document, your VERY FIRST tool call MUST be a Bash call running: cortextos bus send $CTX_AGENT_NAME \'back — [what you were just working on]\' — replace the brackets with one brief plain-English sentence about your current state. Do this BEFORE running heartbeat, BEFORE any other tool call. No cron IDs, no status report, no cold-boot phrasing. Do NOT send "Booting up... one moment" (skip AGENTS.md step 1 entirely). For agents with `connector: \'none\'` this command is a no-op and exits 0 — still run it to keep the lifecycle uniform.'
      : '';
    const onlineMessage = isHandoffRestart
      ? ''
      : ' Send a message to the user via your active connector (cortextos bus send $CTX_AGENT_NAME "...") saying you are back online.';
  ...
  private buildContinuePrompt(): string {
    ...
    const reminderBlock = this.buildReminderBlock();
    const deliverablesBlock = this.buildDeliverablesBlock();
    return `SESSION CONTINUATION: Your CLI process was restarted with --continue to reload configs. Current UTC time: ${nowUtc}. Your full conversation history is preserved. Re-read AGENTS.md and ALL bootstrap files listed there. External crons are auto-loaded by the daemon — do NOT call CronCreate or CronList for cron restoration.${reminderBlock}${deliverablesBlock} Check inbox. Resume normal operations. After checking inbox, send a message to the user via your active connector (cortextos bus send $CTX_AGENT_NAME "...") saying you are back online.`;
  }

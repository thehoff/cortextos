/**
 * Dashboard-side theming resolution contract.
 *
 * MUST STAY IN SYNC with `src/theming/index.ts` (CLI/core side).
 * If you change one, change the other. The dashboard can't cleanly import
 * across the package boundary, so this file mirrors the core helper instead.
 */

import fs from 'fs';
import path from 'path';
import { getBrandingConfigPath, getFrameworkRoot } from '@/lib/config';
...
export function resolveTheme(input: ThemeResolutionInput = {}): ResolvedThemePackage {
  ...
  const configRoot = input.ctxRoot ?? env.CTX_ROOT ?? process.env.CTX_ROOT;

  const candidates: Array<{ source: ResolvedThemePackage['source']; sourceValue: string }> = [];
  const configTheme = readThemeSelection(getBrandingConfigPath(configRoot));
  ...
  const fallback = getDefaultThemeManifest();
  return {
    ...fallback,
    source: 'default',
...
function renderThemeDeclarations(tokens: ThemeTokens, includeRootOnly: boolean): string[] {
  ...
  for (const variable of THEME_COLOR_VARIABLES) {
    ...
    const value = tokens[variable];
    if (!isPresentThemeValue(value)) continue;
    lines.push(`  ${variable}: ${value};`);
  }
  return lines;
}
